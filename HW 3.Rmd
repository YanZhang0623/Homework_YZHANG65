...
```
```{r}            
cor(CS) #find the variables for the model 
library(MASS)
...
```{r}
CS$SAT_AVG <- NULL    # delete the variables we do not need
CS$CONTROL <- NULL
CS$RET_PT4 <- NULL
CS$NPT4_PRIV <- NULL
...
```{r}
set.seed(1234)   #set tranning models
train <- sample(nrow(CS), 0.7*nrow(CS)) 
CS.train <- CS[train,]
CS.validate <- CS[-train,]
dim(CS.train)
dim(CS.validate)
```

```{r}
library(rpart)  #build the decision tree
set.seed(1234)
dtree <- rpart(GRAD_DEBT_MDN_SUPP~.,data=CS.train,method="anova",
