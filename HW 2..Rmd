...
cor(CS)   #find the correlation between each variables
fit1<-lm(CS$GRAD_DEBT_MDN_SUPP~CS$PREDDEG,data=CS)  #build the first model based on the high correlation
summary(fit1)
fit2<-lm(CS$GRAD_DEBT_MDN_SUPP~CS$PREDDEG+CS$CONTROL+CS$LOCALE+CS$RELAFFIL+CS$SAT_AVG+CS$UGDS+CS$PPTUG_EF+CS$NPT4_PUB+CS$NPT4_PRIV
         ...
         +CS$MD_EARN_WNE_P10+CS$GT_25K_P6+CS$GRAD_DEBT_MDN10YR_SUPP+CS$RPY_3YR_RT_SUPP,data=CS)  #build the second model with all variables
